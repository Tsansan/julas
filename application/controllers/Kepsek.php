...
class kepsek extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $data['kepsek']     = $this->kepsek->datakepsek($user, $pword);
    $data['daftarguru'] = $this->kepsek->daftarguru("");
    $data['jumlahguru'] = count($data['daftarguru']);
    $data['jurnalguru'] = $this->kepsek->jurnalguruhariini(date('m-d-Y'));

    // jumlah tidak hadir hari ini
    $countJurnal = count($data['jurnalguru']);
    for ($i = 0; $i < $countJurnal; $i++) {
      # code...
      $sum[$i] = $data['jurnalguru'][$i]['tidakhadir'];
    }

    $sum = array_sum($sum);

    $data['tidakhadir'] = [
      'tanggal' => $data['jurnalguru'][0]['tanggal'],
      'tidakhadir' => $sum
    ];



    $this->load->view("kepsek/auth/header", $data);
    $this->load->view('kepsek/auth/sidebar');
    $this->load->view('kepsek/auth/topbar');
    $this->load->view('kepsek/Home');
    $this->load->view('kepsek/auth/footer');
  }
  ...
  public function jadwal()
  {
    ...
    // datajadwal
    $data['jadwal'] = $this->kepsek->jadwalGuru($search);
    // view jadwal
    $this->load->view("kepsek/auth/header", $data);
    $this->load->view('kepsek/auth/sidebar');
    $this->load->view('kepsek/auth/topbar');
